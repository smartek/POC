...
public class UserSentiment {
	...
	Mapper<LongWritable, Text, Text, IntWritable> {
		...
		public void map(LongWritable key, Text value,
			...
			if (items.length == 9) {
				...
				word.set(date+"\t"+items[2] + "\t" + AnalyseMessage(items[1]));
		...
		public static String AnalyseMessage(String msg) {
			try {
				...
				InputStream in = UserSentiment.class.getClassLoader().getResourceAsStream(sConfigFile);
	...
	public static void main(String[] args) throws Exception {
		JobConf conf = new JobConf(UserSentiment.class);
		conf.setJobName("UserSentiment");
